...
void WindowSwitcher::draw()
{
    ...
    for (size_t index = 0; index < m_windows.size(); ++index) {
        ...
        painter.draw_text(item_rect.translated(thumbnail_width() + 12, 0).translated(1, 1), window.computed_title().bytes_as_string_view(), WindowManager::the().window_title_font(), Gfx::TextAlignment::CenterLeft, text_color.inverted());
        painter.draw_text(item_rect.translated(thumbnail_width() + 12, 0), window.computed_title().bytes_as_string_view(), WindowManager::the().window_title_font(), Gfx::TextAlignment::CenterLeft, text_color);
    ...
    auto add_window_stack_windows = [&](WindowStack& window_stack) {
        window_stack.for_each_window_of_type_from_front_to_back(
            WindowType::Normal, [&](Window& window) {
                ...
                ++window_count;
                longest_title_width = max(longest_title_width, wm.font().width(window.computed_title().bytes_as_string_view()));
