...
int Menu::realize_menu(RefPtr<Action> default_action)
{
    ...
    ConnectionToWindowServer::the().async_create_menu(m_menu_id, MUST(UTF8String::from_ak_string(m_name)));
...
void Menu::realize_menu_item(MenuItem& item, int item_id)
{
    ...
    case MenuItem::Type::Action: {
        ...
        auto shortcut_text = action.shortcut().is_valid() ? MUST(UTF8String::from_utf8(action.shortcut().to_string())) : UTF8String {};
        ...
        ConnectionToWindowServer::the().async_add_menu_item(m_menu_id, item_id, -1, MUST(UTF8String::from_ak_string(action.text())), action.is_enabled(), action.is_checkable(), action.is_checkable() ? action.is_checked() : false, is_default, shortcut_text, icon, exclusive);
        break;
    }
    case MenuItem::Type::Submenu: {
        ...
        ConnectionToWindowServer::the().async_add_menu_item(m_menu_id, item_id, submenu.menu_id(), MUST(UTF8String::from_ak_string(submenu.name())), true, false, false, false, UTF8String {}, icon, false);
        break;
    }
