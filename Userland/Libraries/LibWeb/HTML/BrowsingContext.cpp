...
JS::NonnullGCPtr<BrowsingContext> BrowsingContext::create_a_new_browsing_context(Page& page, JS::GCPtr<DOM::Document> creator, JS::GCPtr<DOM::Element> embedder, BrowsingContextGroup&)
{
    ...
    JS::NonnullGCPtr<SessionHistoryEntry> new_entry = *browsing_context->heap().allocate_without_realm<SessionHistoryEntry>();
    new_entry->url = AK::URL("about:blank");
    new_entry->document_state = *browsing_context->heap().allocate_without_realm<DocumentState>();
    new_entry->document_state->document = document.ptr();
...
JS::NonnullGCPtr<HTML::TraversableNavigable> BrowsingContext::top_level_traversable() const
{
    ...
    auto traversable = active_document()->node_navigable()->top_level_traversable();
    VERIFY(traversable);
...
WebIDL::ExceptionOr<void> BrowsingContext::navigate_to_a_fragment(AK::URL const& url, HistoryHandlingBehavior history_handling, DeprecatedString navigation_id)
{
    ...
    JS::NonnullGCPtr<SessionHistoryEntry> new_entry = *heap().allocate_without_realm<SessionHistoryEntry>();
    new_entry->url = url;
    new_entry->document_state = *heap().allocate_without_realm<DocumentState>();
    new_entry->document_state->document = current_entry().document_state->document;
    new_entry->serialized_state = {};
...
{
    JS::GCPtr<SessionHistoryEntry> entry = m_session_history[entry_index];
...
auto BrowsingContext::create_a_new_auxiliary_browsing_context_and_document(Page& page, JS::NonnullGCPtr<HTML::BrowsingContext> opener) -> BrowsingContextAndDocument
{
...
auto BrowsingContext::create_a_new_browsing_context_and_document(Page& page, JS::GCPtr<DOM::Document> creator, JS::GCPtr<DOM::Element> embedder, JS::NonnullGCPtr<BrowsingContextGroup> group) -> BrowsingContextAndDocument
{
    auto& vm = group->vm();
