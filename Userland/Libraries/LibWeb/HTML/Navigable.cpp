...
WebIDL::ExceptionOr<void> Navigable::navigate(
    ...
    Variant<Optional<POSTResource>, DeprecatedString> document_resource,
    JS::GCPtr<Fetch::Infrastructure::Response> response,
    bool exceptions_enabled,
    HistoryHandlingBehavior history_handling,
    DeprecatedString csp_navigation_type,
    ReferrerPolicy::ReferrerPolicy referrer_policy)
{
    ...
    DeprecatedString navigation_id = "FIXME";
    ...
    if ((document_resource.has<Optional<POSTResource>>() && !document_resource.get<Optional<POSTResource>>().has_value())
        && !response
    ...
    // FIXME: 13. If navigable's ongoing navigation is "traversal", then:

    // FIXME: 14. Set navigable's ongoing navigation to navigationId.
    ...
    document_state->request_referrer_policy = referrer_policy;
    document_state->initiator_origin = initiator_origin_snapshot;
    ...
    // 6. Otherwise, if url is about:srcdoc, then set documentState's origin to navigable's parent's active document's origin.
        ...
        document_state->origin = parent()->active_document()->origin();
    }
...
WebIDL::ExceptionOr<void> Navigable::navigate_to_a_fragment(AK::URL const&, HistoryHandlingBehavior, DeprecatedString navigation_id)
{
    (void) navigation_id;
    TODO();
}

WebIDL::ExceptionOr<void> Navigable::navigate_to_a_javascript_url(AK::URL const&, HistoryHandlingBehavior, Origin const& initiator_origin, DeprecatedString csp_navigation_type)
{
